...
class Profile(models.Model):
    ...
    trust_score = models.IntegerField(default=100) # Scale 0-100
    location_name = models.CharField(max_length=255, blank=True) # e.g., "Lagos, NG"
...
# --- 2. THE CATEGORY TREE (Alibaba Style) ---
class Category(models.Model):
    name = models.CharField(max_length=100)
    slug = models.SlugField(unique=True, blank=True)
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, related_name='children')
    icon = models.CharField(max_length=50, default="fa-solid fa-box", help_text="FontAwesome class")
...
class WholesaleDetail(models.Model):
    ...
    unit_name = models.CharField(max_length=50, help_text="e.g., kg, crate, ton")

class ServiceDetail(models.Model):
    listing = models.OneToOneField(Listing, on_delete=models.CASCADE, related_name='service_data')
    is_available = models.BooleanField(default=True)
    vehicle_info = models.CharField(max_length=100, blank=True, null=True) # For Transport
    location_lat = models.FloatField(null=True, blank=True) # For Hailing
    location_lon = models.FloatField(null=True, blank=True)

# --- 5. THE SOCIAL & NEGOTIATION ENGINE ---
...
# --- 6. SHOPPING CART (Session + Auth Support) ---
class Cart(models.Model):
